...
const NairobiMap = () => {
  ...
  const [legendOpen, setLegendOpen] = useState(false); // ← ADD THIS STATE
    ...
    <section className="py-20 bg-white">
      <div className="max-w-6xl mx-auto px-4">
        ...
        >
          ...
          {/* Collapsible Legend (old legend covered up map on mobile) */}
          <div className={`absolute bottom-4 left-4 bg-white rounded-lg shadow-lg border border-gray-200 transition-all duration-300 ${
            legendOpen ? 'w-48 p-4' : 'w-12 h-12'
          }`}>
            <button 
              onClick={() => setLegendOpen(!legendOpen)}
              className={`absolute -top-2 -right-2 bg-green-600 text-white w-6 h-6 rounded-full text-sm flex items-center justify-center hover:bg-green-700 transition-colors ${
                legendOpen ? '' : 'top-1 right-1'
              }`}
            >
              {legendOpen ? '×' : 'i'}
            </button>
